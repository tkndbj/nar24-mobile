...
class SellerMembershipAndCooperationScreen extends StatefulWidget {
  ...
  const SellerMembershipAndCooperationScreen({
    Key? key,
    this.agreementAssetPath = 'assets/agreements/satici_uyelik_ve_is_ortakligi.txt',
    this.customTitle,
    this.customText,
  }) : super(key: key);

  @override
  State<SellerMembershipAndCooperationScreen> createState() => _SellerMembershipAndCooperationScreenState();
}

class _SellerMembershipAndCooperationScreenState extends State<SellerMembershipAndCooperationScreen> {
  ...
  Future<void> _loadAgreement() async {
    try {
      if (widget.customTitle != null && widget.customText != null) {
        ...
        agreementTitle = lines.firstWhere((line) => line.trim().isNotEmpty, orElse: () => 'Agreement');
