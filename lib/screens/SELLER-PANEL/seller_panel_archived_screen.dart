...
class SellerPanelArchivedScreen extends StatefulWidget {
  const SellerPanelArchivedScreen({super.key});

  @override
  State<SellerPanelArchivedScreen> createState() =>
      _SellerPanelArchivedScreenState();
}

class _SellerPanelArchivedScreenState extends State<SellerPanelArchivedScreen> {
  ...
  Future<void> _loadPausedProducts() async {
    ...
    try {
      Query query = _firestore
          .collection('paused_shop_products')
  ...
  Future<void> _unarchiveProduct(String productId, String productName) async {
    ...
    try {
      final provider = context.read<SellerPanelProvider>();
      await provider.toggleProductPauseStatus(productId, false);
  ...
  void _navigateToUpdateProduct(Product product) {
    ...
    context.push(
      ...
      extra: {
        ...
        'isFromArchivedCollection': true, // NEW FLAG
      },
    );
  }
...
// ✅ Separate widget for product card - better performance
// ✅ Enhanced widget for product card - handles needsUpdate products
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      decoration: BoxDecoration(
        ...
        // Add red border for admin archived products
