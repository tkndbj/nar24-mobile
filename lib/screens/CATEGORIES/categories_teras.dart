...
import '../DYNAMIC-SCREENS/dynamic_teras.dart';
...
class CategoriesTerasScreen extends StatefulWidget {
  const CategoriesTerasScreen({Key? key}) : super(key: key);

  @override
  _CategoriesTerasScreenState createState() => _CategoriesTerasScreenState();
}

class _CategoriesTerasScreenState extends State<CategoriesTerasScreen>
    with RouteAware {
  ...
  Future<void> _fetchProductsForBuyerCategory(String buyerCategory) async {
    ...
    try {
      ...
      // Fetch products using the Teras method in provider (works on 'products' collection)
      // Now with built-in caching
      final products = await marketProvider
          .fetchProductsForBuyerCategoryTeras(buyerCategory);
        ...
        children: [
          ...
          Container(
            ...
            child: ListView.builder(
              ...
              itemCount:
                  localizedBuyerCategories.length + 1, // +1 for Vitrin label
              itemBuilder: (ctx, i) {
                // First item: Vitrin label
                if (i == 0) {
                  return Padding(
                    padding:
                ...
                // Adjust index for actual categories (subtract 1)
                final categoryIndex = i - 1;
                final rawBuyerCategory = buyerCategories[categoryIndex];
                final localizedBuyerCategory =
                    localizedBuyerCategories[categoryIndex];
