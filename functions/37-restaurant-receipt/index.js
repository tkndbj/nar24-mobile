...
export const generateRestaurantReceiptBackground = onDocumentCreated(
  ...
  async (event) => {
    ...
    try {
      ...
      // Re-throw so Cloud Functions retries automatically (up to attempt #3)
...
// ============================================================================
// PDF GENERATOR: thermal-style 80 mm restaurant receipt (Turkish)
// ============================================================================
...
async function generateRestaurantReceipt(data) {
    ...
    try {
      ...
      if (qrPayload) {
        qrImageBuffer = await QRCode.toBuffer(qrPayload, {
          type: 'png',
          width: 200,        // will be scaled down when embedded
          margin: 1,
  ...
  // ── Estimate page height so we don't add blank pages ─────────────────────
  const itemLineCount = (data.items || []).reduce((n, item) => {
    let lines = 1;
    if (item.extras?.length)  lines += Math.ceil(item.extras.length / 2);
    if (item.specialNotes)    lines += 1;
    return n + lines;
  }, 0);

  const estimatedHeight =
      M + 50 +                         // top margin + logo
    8 +                              // dashed line
    (qrImageBuffer ? 110 : 0) +      // QR + label
    26 +                             // payment badge
    8 +                              // dashed
    60 +                             // order number block
    8 +                              // dashed
    50 +                             // buyer name + phone
    (data.deliveryType !== 'pickup' && data.deliveryAddress ? 60 : 20) + // address
    (data.orderNotes ? 50 : 0) +     // notes
    16 +                             // dashed + items header
    itemLineCount * 20 +             // item rows
    (data.items?.length || 0) * 18 + // base item height
    60 +                             // totals
    50 +                             // date + footer
    M;

  const PAGE_HEIGHT = Math.max(estimatedHeight, 400);
  ...
  return new Promise((resolve, reject) => {
    ...
    // ── Register fonts ────────────────────────────────────────────
    // Uses the same Inter fonts already present in your functions/fonts/ dir
    ...
    const twoCol = (label, value, yPos, {
      ...
      labelColor = '#777',
      valueColor = '#000',
      split      = 0.42,       // fraction of CW for label column
    } = {}) => {
      const lW = CW * split;
      const vW = CW * (1 - split);
    ...
    // ── Micro-label (small grey caps label) ──────────────────────
    ...
    try {
      ...
      // Logo file missing → text fallback
    ...
    if (data.deliveryType !== 'pickup' && data.deliveryAddress) {
      ...
      // Show address phone only if different from buyer phone
    ...
    for (const item of data.items || []) {
      ...
      // Special note (indented, smallest)
      ...
      y += 4; // breathing room between items
    }
    ...
    // Show subtotal + delivery fee only when fee > 0
    ...
    if (deliveryFee > 0) {
      ...
      y = twoCol(
        'Teslimat Ucreti',
        `${deliveryFee.toFixed(0)} ${data.currency}`,
        y,
      );
      y = dashed(y);
      y += 2;
